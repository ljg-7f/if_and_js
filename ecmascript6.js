...
// var 全局变量 = 顶层对象的属性；let&const 只是全局变量而已。
...
[1, undefined, 3].map((x = 'yes') => x); // [ 1, 'yes', 3 ] 默认值
[1, , 3].map((x = 'yes') => x); // [ 1, 3 ]
// 遍历map
...
String.codePointAt() // 返回指定位置的字符的完整Unicode编码（一个整数，可以大于65535）。如果你在字符串中的位置指向一个代理对的开头，它会正确地将这两个编码单元合并，返回该字符完整的Unicode值。
String.charCodeAt() // 返回指定位置的字符的Unicode编码（一个 0 到 65535 之间的整数）。如果字符是由两个编码单元组成的（如 '😊' = 0xD83D + 0xDE0A），charCodeAt 只能返回第一个编码单元的值（即高位代理对），而不是整个字符的完整代码点。
String.charAt() // 返回指定位置的字符本身（一个字符串），但对于那些占用两个编码单元的Unicode字符（例如 '😊'），它无法正确返回整个字符，而是返回该字符的前半部分（一个无效的乱码或字符片段）。
String.at(-1) // 倒数
...
typeof 123n // 'bigint'
1n + 1.3 // 报错：BigInt 不能与普通数值进行混合运算，但可以比较大小
...
Object.defineProperties(target, Object.getOwnPropertyDescriptors(source)); // 将source的所有属性描述符全部defineProperties定义到target上
